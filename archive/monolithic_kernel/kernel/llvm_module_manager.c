...
void llvm_module_export_profile(llvm_module_manager_t* mgr, int module_id) {
    ...
    serial_puts("# Current Optimization Level: O");
    print_int(mod->current_level);
    serial_puts("\n");

    // Output in simplified LLVM profdata format
    // Format: function_name:call_count:total_cycles
    serial_puts(mod->name);
    serial_puts(":compute:");
    print_int((int)mod->call_count);
    serial_puts(":");
    ...
    serial_puts("# Hotness Score: ");
    if (mod->call_count >= 10000) {
        serial_puts("VERY_HOT (>=10000 calls)");
    } else if (mod->call_count >= 1000) {
        serial_puts("HOT (>=1000 calls)");
    } else if (mod->call_count >= 100) {
        serial_puts("WARM (>=100 calls)");
    } else {
        serial_puts("COLD (<100 calls)");
    }
    serial_puts("\n");
}
...
void llvm_module_export_all_profiles(llvm_module_manager_t* mgr) {
    ...
    serial_puts("\n");
    serial_puts("========================================================================\n");
    serial_puts("=== LLVM PGO PROFILE EXPORT ===\n");
    serial_puts("========================================================================\n");
    serial_puts("# Generated by BareFlow LLVM Module Manager\n");
    serial_puts("# Total Modules: ");
    print_int((int)mgr->module_count);
    serial_puts("\n");
    serial_puts("# Format: module:function:call_count:total_cycles\n");
    serial_puts("========================================================================\n");

    for (uint32_t i = 0; i < mgr->module_count; i++) {
        llvm_module_export_profile(mgr, i);
    }

    serial_puts("========================================================================\n");
    serial_puts("=== END OF PROFILE DATA ===\n");
    serial_puts("========================================================================\n");
}
